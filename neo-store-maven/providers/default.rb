...
def create_command_string(artifact_file, new_resource)
  ...
  dest = '-Ddest=' + artifact_file #extra support link - 
  ...
  plugin_version = '3.4'
  plugin = "org.apache.store-maven.plugins:store-maven-dependency-plugin:#{plugin_version}:get"
...
def get_mvn_artifact(action, new_resource)
  ...
  Dir.mktmpdir('chef_store-maven_lwrp') do |tmp_dir|
